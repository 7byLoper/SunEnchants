...
public class MegaBulldozerEnchant extends SEnchant {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        if (!tool.hasItemMeta() || !isApplied(tool) || enchantsManager.hasEnchant("bulldozer_log", tool)) {
            return;
        }
        ...
        int brokenBlocksCount = radius == 1
                ? bulldozerUtils.breakBlocksInPlane(
                        centerBlock, tool, player, useXZ, useXY, useYZ, collectedItems, processedBlocks)
                : bulldozerUtils.breakBlocksInCube(centerBlock, tool, player, radius, collectedItems, processedBlocks);

        bulldozerUtils.applyToolDamage(tool, brokenBlocksCount);
